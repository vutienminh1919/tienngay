...
class Sms extends REST_Controller{
    ...
    public function report_sms_month_post()
    {
       $flag = notify_token($this->flag_login);
        if ($flag == false) return;
     
          $data = $this->input->post();
        $fdate_export = !empty($data['fdate_export']) ? $data['fdate_export'] : "";
      

        $month = date('m', strtotime($fdate_export)); // 2020-01-01
        $year = date('Y', strtotime($fdate_export)); // 2020-01-31

        $condition = array();
        if (!empty($month)) $condition['month'] = $month;
        if (!empty($year)) $condition['year'] = $year;

        $contract = $this->report_sms_month_model->getMonth($condition);
    ...
    public function find_where_not_in_post() {
        ...
        $data = $this->input->post();
        unset($data['type']);
        $faqs = $this->sms_model->find_where_not_in($data['where'], $data['fields'], convertToMongoObject($data['not_in']));
        $response = array(
            'status' => REST_Controller::HTTP_OK,
            'data' => $faqs
        );
    ...
    public function find_where_post() {
        ...
        $data = $this->input->post();
        unset($data['type']);
        $faqs = $this->sms_model->find_where_select($data, array("_id", "name", "province", "district", "address"));
        if (!empty($faqs)) {
        	foreach ($faqs as $sto) {
    ...
    public function get_all_post(){
        $flag = notify_token($this->flag_login);
        if ($flag == false) return;
        $faq = $this->sms_model->find_where(['status'=>['$in'=>['new']]]);
    ...
    public function get_all_home_post(){
        ...
        $faq = $this->sms_model->find_where('status', ['active']);
        if (!empty($faq)) {
            foreach ($faq as $s) {
                $s['id'] = (string)$s['_id'];
            }
    ...
    public function get_faq_post(){
        ...
        $faq = $this->sms_model->findOne(array("_id" => new MongoDB\BSON\ObjectId($id)));
        $response = array(
            'status' => REST_Controller::HTTP_OK,
            'data' => $faq
        );
     ...
     public function get_description_faq_post(){
        ...
        $faq = $this->sms_model->findOne(array("link" => $link));
        $response = array(
            'status' => REST_Controller::HTTP_OK,
            'data' => $faq
        );
    ...
    public function update_faq_post(){
        ...
        $data = $this->input->post();
        $id = !empty($data['id']) ? $data['id'] : "";
        $count = $this->sms_model->count(array("_id" => new \MongoDB\BSON\ObjectId($id)));
        if($count != 1) {
            $response = array(
                'status' => REST_Controller::HTTP_UNAUTHORIZED,

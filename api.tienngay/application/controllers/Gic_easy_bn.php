...
class Gic_easy_bn extends REST_Controller
...
{	public function __construct()
	...
	public function insert_gic_easy_post()
	{
		// $flag = notify_token($this->flag_login);
		// if ($flag == false) return;
		date_default_timezone_set('Asia/Ho_Chi_Minh');
		$data = $this->input->post();
		$province_name = !empty($data['province_name']) ? $data['province_name'] : '';
		$district_name = !empty($data['district_name']) ? $data['district_name'] : '';
		$nhan_hieu = !empty($data['nhan_hieu']) ? $data['nhan_hieu'] : '';
		...
		$ho_ten_chu_xe = !empty($data['ho_ten_chu_xe']) ? $data['ho_ten_chu_xe'] : '';
		$dia_chi_dang_ky = !empty($data['dia_chi_dang_ky']) ? $data['dia_chi_dang_ky'] : '';
		$code_gic_easy = !empty($data['code_gic_easy']) ? $data['code_gic_easy'] : "";
		...
		if (!preg_match("/^[A-Z0-9]{7,9}$/", $bien_so_xe)) {
			$response = array(
				'status' => REST_Controller::HTTP_UNAUTHORIZED,
		...
		$type_gic = "GIC_EASY_BN";
	
			$NGAY_HL = date('Y-m-d H:i:s', strtotime("+1 days"));
			$NGAY_KT = "";
		
		$code = "GIC_EASY_" . date("dmY") . "_" . time();
		$gic_easy = $this->insert_gic_easy($data, $NGAY_HL, $NGAY_KT, $type_gic,$code);
		
		if ($gic_easy->success != true) {
			$mes = $gic_easy->message;
			$response = array(
				'status' => REST_Controller::HTTP_UNAUTHORIZED,
				'message' => $mes
			);
			...
			$this->gic_easy_bn_model->insert($dt_gic);
			$response = array(
	...
	public function log_gic_easy($request, $response, $type, $code)
	{
	...
	public function insert_gic_easy($data, $NGAY_HL, $NGAY_KT,$type,$code)
	{
		date_default_timezone_set('Asia/Ho_Chi_Minh');
		...
		$houseHold_address = (!empty($data['address'])) ? $data['address'] : '';
		...
		$nhanhieu = $data['nhan_hieu'];
	
		$model = $data['model'];
	
		$biensoxe = $data['bien_so_xe'];
	
		$sokhung = $data['so_khung'];
	
		$somay = $data['so_may'];
	
		$hotenchuxe = $data['ho_ten_chu_xe'];
	
		$diachidangky = $data['dia_chi_dang_ky'];
	

		
		$amount_GIC_easy = (!empty($data['price'])) ? $data['price'] : 0;
		$code_GIC_easy = (!empty($data['code_gic_easy'])) ? $data['code_gic_easy'] : "";
		if (!empty($city['city'])) {
		...
		$type_gic = $code_gic;
		$this->log_gic_easy(json_encode($dt_gic), $res, $type, $code);
		//return $res;
	...
	public function get_list_gic_easy_post()
	{
		// $flag = notify_token($this->flag_login);
		// if ($flag == false) return;
		$data = $this->input->post();
		$tab = !empty($data['tab']) ? $data['tab'] : 'gic_easy';
		...
		$code_gic_easy = !empty($data['code_gic_easy']) ? $this->security->xss_clean($data['code_gic_easy']) : '';
		...
		if (!empty($code_gic_easy)) {
			$condition['code_gic_easy'] = $code_gic_easy;
		}
		...
		if ($tab == 'gic_easy') {
			$result = $this->gic_easy_bn_model->get_list_gic_easy($condition, $per_page, $uriSegment);
			$total = $this->gic_easy_bn_model->get_count_gic_easy($condition);
		} else {
			$result = $this->transaction_model->list_transaction_gic_easy($condition, $per_page, $uriSegment);
			$total = $this->transaction_model->total_list_transaction_gic_easy($condition);
		}
		$response = array(
			'status' => REST_Controller::HTTP_OK,
			'data' => $result,
	...
	public function statistical_gic_easy_post()
	{
		$flag = notify_token($this->flag_login);
		if ($flag == false) return;
		$data = $this->input->post();
	...
	{
		$data = [];
		$mics = $this->gic_easy_bn_model->get_list_gic_store($condition);
		if (!empty($mics)) {
			$sum_price = 0;
			foreach ($mics as $mic) {
				$sum_price += $mic['price'];
   ...
   private function push_api_gci($action = '', $get = '', $data_post = [])
	...
	public function detail_gic_easy_post()
	{
		$data = $this->input->post();
		$id = !empty($data['id']) ? $data['id'] : '';
		$data = $this->gic_easy_bn_model->findOne(['_id' => new MongoDB\BSON\ObjectId($id)]);
		$response = array(
			'status' => REST_Controller::HTTP_OK,
			'data' => $data,
			'message' => "thành công!"
	...
	public function get_gic_easy_accounting_transfe_post()
	{
		$flag = notify_token($this->flag_login);
		if ($flag == false) return;
		$condition = [];
		...
		$mic = $this->gic_easy_bn_model->get_gic_easy_accounting_transfe($condition);
		$total_money = 0;
	...
	public function create_transaction_post()
	{
		...
		foreach ($code_gic as $value) {
			$gic_easy = $this->gic_easy_bn_model->findOne(['gic_code' => $value]);
			$this->gic_easy_bn_model->update(['_id' => $gic_easy['_id']], ['receipt_code' => $code, 'status' => 2]);
			$money += (int)$gic_easy['price'];
		}
		$data_transaction = [
			'code' => $code,
			'total' => (string)$money,
	...
	public function detail_transaction_post()
	{
		...
		$mic = $this->gic_easy_bn_model->find_where(['receipt_code' => $code]);
		$response = array(
			'status' => REST_Controller::HTTP_OK,
			'data' => $mic
		);
	...
	public function get_total_pay_post()
	{
		...
		foreach ($code as $value) {
			$mic = $this->gic_easy_bn_model->findOne(['gic_code' => $value]);
			$total += (int)$mic['price'];
		}
		$response = array(
			'status' => REST_Controller::HTTP_OK,

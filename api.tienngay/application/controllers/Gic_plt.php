...
class Gic_plt extends REST_Controller{
    ...
    public function find_where_not_in_post() {
        ...
        unset($data['type']);
        $gics = $this->gic_plt_model->find_where_not_in($data['where'], $data['fields'], convertToMongoObject($data['not_in']));
        $response = array(
    ...
    public function find_where_post() {
        ...
        $data = $this->input->post();
        unset($data['type']);
        $gics = $this->gic_plt_model->find_where_select($data, array("_id", "name", "province", "district", "address"));
        if (!empty($gics)) {
            foreach ($gics as $sto) {
    ...
    public function get_all_post(){
		...
		// }
        $gic = $this->gic_plt_model->getGic($condition,$per_page , $uriSegment);
        $total = $this->gic_plt_model->getGic($condition,$per_page , $uriSegment, true);
        $response = array(
            'status' => REST_Controller::HTTP_OK,
            'data' => $gic,
            'total' => $total,
    ...
    public function get_all_home_post(){
        ...
        $gic = $this->gic_plt_model->find_where('status', ['active']);
        if (!empty($gic)) {
            foreach ($gic as $s) {
                $s['id'] = (string)$s['_id'];
            }
    ...
    public function get_one_post(){
        ...
        $gic = $this->gic_plt_model->findOne(array("_id" => new MongoDB\BSON\ObjectId($id)));
        $response = array(
            'status' => REST_Controller::HTTP_OK,
            'data' => $gic
        );
    ...
    //lấy danh danh contract bằng mã nhà đầu tư
     ...
     public function get_description_gic_post(){
        ...
        $gic = $this->gic_plt_model->findOne(array("link" => $link));
        $response = array(
            'status' => REST_Controller::HTTP_OK,
            'data' => $gic
        );
    ...
    public function create_gic_post(){
        ...
        $this->gic_plt_model->insert($data);
        $response = array(
            'status' => REST_Controller::HTTP_OK,
    ...
    public function update_gic_post(){
        ...
        $data = $this->input->post();
        $id = !empty($data['id']) ? $data['id'] : "";
        $count = $this->gic_plt_model->count(array("_id" => new \MongoDB\BSON\ObjectId($id)));
        if($count != 1) {
            $response = array(
                'status' => REST_Controller::HTTP_UNAUTHORIZED,

...
class Vbi_utv extends REST_Controller
{
	...
	public function get_list_vbi_utv_post()
	{
		$flag = notify_token($this->flag_login);
		if ($flag == false) return;
		$data = $this->input->post();
		$tab = !empty($data['tab']) ? $data['tab'] : 'vbi_utv';
		...
		if ($tab == 'vbi_utv') {
			$result = $this->vbi_utv_model->get_list_vbi_utv($condition, $per_page, $uriSegment);
			$total = $this->vbi_utv_model->get_list_vbi_utv($condition, $per_page, $uriSegment, true);
		} else {
			$result = $this->transaction_model->list_transaction_vbi_utv($condition, $per_page, $uriSegment);
			$total = $this->transaction_model->total_list_transaction_vbi_utv($condition);
		}
		$response = array(
			'status' => REST_Controller::HTTP_OK,
			'data' => $result,
	...
	public function fees_apply_post()
	{
		...
		if ($result->response_code == '00') {
			...
			$this->vbi_utv_model->insert($insert);
			$result = [
				'status' => REST_Controller::HTTP_OK,
				'message' => 'Thành công!',
			];
	...
	public function check_birthday_post()
	{
		...
		if (empty($age)) {
			...
			if ($age < 18 || $age > 65) {
				$result = [
					'status' => REST_Controller::HTTP_UNAUTHORIZED,
					'message' => 'Độ tuổi người được bảo hiểm tối thiểu 18 tuổi và tối đa 65 tuổi!'
				];
				$this->set_response($result, REST_Controller::HTTP_OK);
				return;
			} else {
				$list_bh = $this->lay_danh_sach_bao_hiem_utv();
				$goi_bh = [];
				foreach ($list_bh as $value) {
					if ($value->tu_tuoi <= $age && $value->toi_tuoi >= $age) {
	...
	private function lay_danh_sach_bao_hiem_utv()
	{
		$vbi = new BaoHiemVbi();
		$data = $vbi->danh_sach_bh_utv();
		return $data->goi_bh;
	}
	...
	public function get_price_goi_bh_post()
	{
		...
		if (empty($age)) {
			...
			if ($age < 18 || $age > 65) {
				...
				$list_bh = $this->lay_danh_sach_bao_hiem_utv();
				foreach ($list_bh as $value) {
					if ($value->tu_tuoi <= $age && $value->toi_tuoi >= $age && $goi_bao_hiem == $value->ma) {
						$price = $value->phi_bh;
					}
				}
				$result = [
	...
	public function get_vbi_utv_accounting_transfe_post()
	{
		$flag = notify_token($this->flag_login);
		if ($flag == false) return;
		$condition = [];
		...
		$vbi = $this->vbi_utv_model->get_vbi_utv_accounting_transfe($condition);
		$total_money = 0;
	...
	public function create_transaction_post()
	{
		...
		foreach ($code_vbi as $value) {
			$vbi_utv = $this->vbi_utv_model->findOne(['code' => $value]);
			$this->vbi_utv_model->update(['_id' => $vbi_utv['_id']], ['receipt_code' => $code, 'status' => 2]);
			$money += (int)$vbi_utv['fee'];
		}
		$data_transaction = [
			'code' => $code,
			'total' => (string)$money,
	...
	public function detail_transaction_post()
	{
		...
		$vbi = $this->vbi_utv_model->find_where(['receipt_code' => $code]);
		$response = array(
			'status' => REST_Controller::HTTP_OK,
			'data' => $vbi
		);
	...
	public function get_total_pay_post()
	{
		...
		foreach ($code as $value) {
			$vbi = $this->vbi_utv_model->findOne(['code' => $value]);
			$total += (int)$vbi['fee'];
		}
		$response = array(
			'status' => REST_Controller::HTTP_OK,

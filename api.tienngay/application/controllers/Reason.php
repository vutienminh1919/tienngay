...
class Reason extends REST_Controller{
	...
	public function get_all_post(){
		$flag = notify_token($this->flag_login);
		if ($flag == false) return;
		$reason = $this->reason_model->find_where('status', ['active','deactive']);
		if (!empty($reason)) {
			foreach ($reason as $r) {
				$r['id'] = (string)$r['_id'];
			}
	...
	public function get_reason_post(){
		...
		if(empty($id)){
			$response = array(
				...
				'message' => "Id reason already exists"
			);
			$this->set_response($response, REST_Controller::HTTP_OK);
			return;
		}
		$reason = $this->reason_model->findOne(array("_id" => new MongoDB\BSON\ObjectId($id)));
		$response = array(
			'status' => REST_Controller::HTTP_OK,
			'data' => $reason
		);
	...
	public function create_reason_post(){
		$flag = notify_token($this->flag_login);
		if ($flag == false) return;
		$data = $this->input->post();
		$data['code_reason'] = $this->initNumberReasonCode();
		$this->reason_model->insert($data);
		$response = array(
			'status' => REST_Controller::HTTP_OK,
			'message' => "Create reason success",
			'data' => $data
		);
	...
	public function update_reason_post(){
		$flag = notify_token($this->flag_login);
		if ($flag == false) return;
		$data = $this->input->post();
		$id = !empty($data['id']) ? $data['id'] : "";
		$count = $this->reason_model->count(array("_id" => new \MongoDB\BSON\ObjectId($id)));
		if($count != 1) {
			$response = array(
				'status' => REST_Controller::HTTP_UNAUTHORIZED,
				'message' => "Không tồn tại lý do nào cần cập nhật"
			);
			$this->set_response($response, REST_Controller::HTTP_OK);
			return;
		}
		$this->log_reason($data);
		unset($data['id']);

		$this->reason_model->update(
			array("_id" => new MongoDB\BSON\ObjectId($id)),
			$data
		);
		$response = array(
			'status' => REST_Controller::HTTP_OK,
			'message' => "Update reason success",
			'data' => $data
		);
		$this->set_response($response, REST_Controller::HTTP_OK);
		return;
	...
	public function log_reason($data){
		$id = !empty($data['id']) ? $data['id'] : "";
		$reason = $this->reason_model->findOne(array("_id" => new MongoDB\BSON\ObjectId($id)));
		$reason['id'] = (string)$reason['_id'];
		unset($reason['_id']);
		$dataInser = array(
			"new_data" => $data,
			"old_data" => $reason,
	...
	private function initNumberReasonCode()
	{
		$maxNumber = $this->reason_model->getMaxNumberReason();
		$maxNumberReason = !empty($maxNumber[0]['code_reason']) ? (int)$maxNumber[0]['code_reason'] + 1 : 1;
		return $maxNumberReason;
	}
}
?>

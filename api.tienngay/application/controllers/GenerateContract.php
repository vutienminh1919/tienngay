...
class GenerateContract extends REST_Controller
{
...
//			return;
		...
		if (!empty($contract)) {
			...
			if ($down_interest_on_month == 'active' || $is_reduction_interest == 'active') {
				$date_pay_input = !empty($this->dataPost['date_pay']) ? strtotime(date('Y-m-d', $this->dataPost['date_pay']) . ' 23:59:59') : '';
				$date_pay = !empty($date_pay_input) ? $date_pay_input : strtotime(date('Y-m-d') . ' 23:59:59');
				$check_date_pay_late = !empty($contract['interest_reduction']['isset_date_late']) ? $contract['interest_reduction']['isset_date_late'] : '';
				// Nếu thanh toán quá hạn hoặc phát sinh số ngày chậm trả => không được hưởng ưu đãi coupon
				if ( !empty($contract['expire_date']) && !empty($date_pay) && ( ($date_pay > strtotime(date('Y-m-d', $contract['expire_date']) . ' 23:59:59')) || $check_date_pay_late == true) ) {
			...
			//cập nhật lại trường debt contract
...
//			return;
		...
		if ($type_interest == 1) {
			...
			for ($i = 1; $i <= $so_ky_vay; $i++) {
				...
				if ($is_reduction_interest == "active") {
					...
					if ($ky_tra == $so_ky_vay) {
						// trừ tiền coupon có giảm lãi 3 tháng đầu vào kỳ cuối
	...
	public function getMoneyPerMonth_post()
	{
//		$amount_money  tong tien
//		$type_loan  hinh thuc vay
//		$number_day_loan  tong so ngay vay
...
//		$insurrance  bao hiem
		...
		$insurrance = $this->dataPost['insurrance'];
		$type_interest = $this->dataPost['type_interest'];
		$period_pay_interest = $this->dataPost['period_pay_interest'];
		$number_day_loan = $this->dataPost['number_day_loan'];
		$amount_money = $this->dataPost['amount_money'];
		$code_contract = $this->dataPost['code_contract'];
		// get thông tin phí vay
		...
		if ($type_interest == 1) {
			...
			$tong_phi_lai = $phi_tu_van + $phi_tham_dinh + $lai_ky;
			if ($so_ky_vay == 1) {
				$tong_phi_lai = $tong_phi_lai + $tien_goc;
			}
			$response = array(
				'status' => REST_Controller::HTTP_OK,
				'data' => $tong_phi_lai

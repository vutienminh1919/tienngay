...
class Reason_model extends CI_Model
{
	...
	public function find_reason_in($condition, $in)
	{
		return $this->mongo_db->where($condition)->where_in('code_reason', $in)->order_by(array('created_at' => 'DESC'))->limit('1')->get($this->collection);
	}
	...
	public function getMaxNumberReason()
	{
		return $this->mongo_db
			->select(array("code_reason"))
			->order_by(array('code_reason' => 'DESC'))
			->limit(1)
			->get($this->collection);
	}

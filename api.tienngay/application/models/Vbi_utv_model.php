...
class Vbi_utv_model extends CI_Model
{
	private $collection = 'vbi_utv';
	...
	public function get_list_vbi_utv($condition, $limit, $offset, $total = false)
	{
		...
		if (!empty($condition['filter_by_store'])) {
	...
	public function get_count_vbi_utv($condition)
	{
		$where = array();
		$mongo = $this->mongo_db;
	...
	public function get_vbi_utv_accounting_transfe($condition)
	{
		$order_by = ['created_at' => 'DESC'];
		$where = array();
		$mongo = $this->mongo_db;
	...
	public function getVbiUtvNotYetSendAll($condition)
	{
	...
	public function getVbiUtvSendDay($condition)
	{
		if ((!empty($condition['type_transaction']) && !in_array($condition['type_transaction'], [11])) || (!empty($condition['status']) && $condition['status'] != 10)) return;
		$order_by = ['created_at' => 'ASC'];
		$where = array();
		$mongo = $this->mongo_db;
	...
	public function find_by_select($condition)
	{
		...
		return $mongo->select(['status', 'fee'])
			->get($this->collection);
	}
	...
	public function getAll_excel($condition){
		...
		return $mongo->order_by($order_by)
			->select(['code','fee','NGAY_HL','NGAY_KT','customer_info','store.name','vbi_utv','created_at','created_by','goi_bh'])
			->get($this->collection);

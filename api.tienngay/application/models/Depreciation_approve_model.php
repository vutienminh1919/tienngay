...
class Depreciation_approve_model extends CI_Model
{
	private $collection = 'depreciation_pending_approve';
	private $manager;
	...
	public function get_depreciation_new($condition = array(), $limit = 30, $offset = 0)
	{
		...
		return $mongo
			->order_by(array('status' => 1))
			->order_by(['created_at' => 'DESC'])
			->order_by(array('slug_main_property' => 'ASC'))
	...
	public function get_count_pending_depreciation($condition)
	{
		$mongo = $this->mongo_db;
		$where = array();
		if (!empty($condition['hang_xe_khau_hao'])) {

...
class Gic_easy_bn_model extends CI_Model
{
	private $collection = 'gic_easy_bn';
	...
	public function get_list_gic_easy($condition, $limit, $offset)
	{
		...
		if (isset($condition['code_gic_easy'])) {
			$where['gic_code'] = $condition['code_gic_easy'];
		}
	...
	public function get_count_gic_easy($condition)
	{
		$where = array();
		$mongo = $this->mongo_db;
		if (!empty($condition['start']) && !empty($condition['end'])) {
	...
	public function get_gic_easy_accounting_transfe($condition)
	{
		$order_by = ['created_at' => 'DESC'];
		$where = array();
		$mongo = $this->mongo_db;
	...
	public function getGicEasyNotYetSendAll($condition)
	{
		$order_by = ['created_at' => 'ASC'];
		$where = array();
		$mongo = $this->mongo_db;
	...
	public function getGicEasySendDay($condition)
	{
		if ((!empty($condition['type_transaction']) && !in_array($condition['type_transaction'], [13])) || (!empty($condition['status']) && $condition['status'] != 10)) return;
		$order_by = ['created_at' => 'ASC'];
		$where = array();
		$mongo = $this->mongo_db;

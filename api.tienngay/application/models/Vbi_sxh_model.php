...
class Vbi_sxh_model extends CI_Model
{
	private $collection = 'vbi_sxh';
	...
	public function get_list_vbi_sxh($condition, $limit, $offset, $total = false)
	{
	...
	public function get_count_vbi_sxh($condition)
	{
		$where = array();
		$mongo = $this->mongo_db;
	...
	public function get_vbi_sxh_accounting_transfe($condition)
	{
		$order_by = ['created_at' => 'DESC'];
		$where = array();
		$mongo = $this->mongo_db;
	...
	public function getVbiSxhNotYetSendAll($condition)
	{
	...
	public function getVbiSxhSendDay($condition)
	{
		if ((!empty($condition['type_transaction']) && !in_array($condition['type_transaction'], [12])) || (!empty($condition['status']) && $condition['status'] != 10)) return;
		$order_by = ['created_at' => 'ASC'];
		$where = array();
		$mongo = $this->mongo_db;
	...
	public function getAll_excel($condition){
		...
		return $mongo->order_by($order_by)
			->select(['code','fee','NGAY_HL','NGAY_KT','customer_info','store.name','vbi_sxh','created_at','created_by','goi_bh'])
			->get($this->collection);

...
class Report_contract_loan_model extends CI_Model
{
	private $collection = 'report_contract_loan';
	...
	public function find_where_home($condition=array()){
		$order_by = ['created_at' => 'DESC'];
		$where = array();
		$in = array();
		$mongo = $this->mongo_db;
        if(isset($condition['created_at'])) {
			//$where['start_date'] = array('$lte' => $condition['created_at']);
			$where['end_date'] = array('$gte' => $condition['created_at']);
		}
    ...
    public function drop_collection(){
		return $this->mongo_db->drop_collection($this->collection);
	}

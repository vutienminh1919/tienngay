...
class Contract_ksnb_model extends CI_Model
{

	private $collection = 'contract_ksnb';
	...
	public function getDataByRole($condition = array(), $limit = 30, $offset = 0)
	{
		...
		if (isset($condition['store'])) {
			$where['contract.store.id'] = $condition['store'];
		}
		...
		if (!empty($condition['code_contract'])) {
			$mongo = $mongo->like("contract.code_contract", $condition['code_contract']);
		}
		if (!empty($condition['code_contract_disbursement'])) {
			$mongo = $mongo->like("code_contract_disbursement_value", $condition['code_contract_disbursement']);
		}
		if (!empty($condition['customer_name'])) {
			$mongo = $mongo->like("contract.customer_infor.customer_name", $condition['customer_name']);
		}
		if (!empty($condition['customer_phone_number'])) {
			$mongo = $mongo->like("contract.customer_infor.customer_phone_number", $condition['customer_phone_number']);
		}
		if (!empty($condition['customer_identify'])) {
			$mongo = $mongo->like("contract.customer_infor.customer_identify", $condition['customer_identify']);
		}

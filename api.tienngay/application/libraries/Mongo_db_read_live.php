...
//die($param);
	...
	function __construct()
	{
		...
		$this->config = $this->CI->config->item('mongo_db_read_live');
...
//            $this->connect->close();
	...
	{
//    	var_dump($this->config['active']);die;
		...
		{
			show_error("MongoDB configuration is missing. 123", 500);
		}
		if(isset($this->config[$this->activate]) == TRUE)
		{
			...
			if(isset($this->config[$this->activate]['admin']) && !empty($this->config[$this->activate]['admin'])){
				$this->admin = $this->config[$this->activate]['admin'];
			}
		}
		else
		{
	...
	private function connect()
	{
		...
		try
		{
			$dsn = "mongodb://".$this->username.":".$this->password."@";
			...
			$dsn .= "/{$this->database}";
			if(isset($this->config[$this->activate]['no_auth']) && $this->config[$this->activate]['no_auth'] == TRUE)
			{
				$options = array();
			}
			else
			{
				$options = array('username'=>$this->username, 'password'=>$this->password, 'authSource'=>$this->admin);
			}
	...
	public function insertReturnId($collection = "", $insert = array()) {
		$insertOneResult = $this->db->selectCollection($collection)->insertOne($insert, array('w' => $this->write_concerns, 'j'=>$this->journal));
	 ...
	 *
	 * @usage : $this->mongo_db->find_one_and_update('foo');
	 */

	public function find_one_and_update($collection="", $filter="", $update="") {
		return $this->db->selectCollection($collection)->findOneAndUpdate($filter, $update);
	}
	...
	public function count($collection = "")
	{
		...
		$options = ['limit'=>(int) $this->limit, 'skip'=>(int) $this->offset, 'sort'=>$this->sorts, 'batchSize'=>(int)$this->limit, 'cursorType'=>3];
	...
	function where_text($text, $options = [])
	{
		$text = quotemeta($text);
		...
		if (isset($text) && $text != '')
		{
			...
			$this->push_where_field('$text', ['$search' => "\"".$text."\""] + $this->get_text_options($options));
		}

		return $this;
	}

	function push_where_field($field = '', $data)
	{
		if ($field != '' && isset($data))
		{
	...
	function get_text_options(array $options = [])
	{
		$result = [];
	...
	function create_where_field($field = '')
	{
	...
	public function where_in_like($fields, array $list = [])
	{

...
class PaymentProcess extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'vpbank:paymentProcess';
     ...
     * @var string
     */
    protected $description = "create transaction of contract";
    ...
    public function handle()
    {
        ...
        Log::channel('cronjob')->info('Run VPBank PaymentProcess job');
        $transactions = DB::table('vpbank_transactions')
                ...
                ->whereNotNull(VPBankTransaction::VIRTUAL_ACCOUNT_NUMBER) // không null van
                ->where(VPBankTransaction::VIRTUAL_ACCOUNT_NUMBER, '!=', '') // không empty van
        ...
        if ($transactions->count() < 1) {
            ...
            Log::channel('cronjob')->info('Run VPBank PaymentProcess job success');
            return;
        }
        //get only IDs value
        ...
        foreach ($transactions as $key => $value) {
            ...
            if ($this->isExistedBankCode($value[VPBankTransaction::TRANSACTION_ID])) {
                Log::channel('cronjob')->info('Run VPBank PaymentProcess bankCode has been inserted already! ' . $value[VPBankTransaction::TRANSACTION_ID]);
                continue;
            }
            // process payment
            $vpController = App::make(\Modules\VPBank\Http\Controllers\VPBankController::class);
            Log::channel('cronjob')->info('Run VPBank PaymentProcess run process payment ' . $value[VPBankTransaction::ID]);
            $vpController->callAction('processPayment', [$value[VPBankTransaction::ID], 'cronjob']);
        }
        Log::channel('cronjob')->info('Run VPBank PaymentProcess job success');
    }

    protected function isExistedBankCode($bankCode) {
        $transactions = Transaction::where(Transaction::CODE_TRANSACTION_BANK, $bankCode)
            ->whereIn(Transaction::STATUS, [Transaction::STATUS_WAIT_CONFIRM, Transaction::STATUS_SUCCESS])
            ->get();

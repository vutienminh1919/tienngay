...
use Modules\MongodbCore\Entities\ReportForm3;
...
class ReportForm3Repository implements ReportForm3RepositoryInterface
{
    ...
    /**
     * ReportForm3Repository constructor.
     *
     * @param ReportForm3 $rpModel
     */
    public function __construct(ReportForm3 $rpModel) {
    ...
    public function getListByMonth($time) {
        ...
        $reports = $this->rpModel::where(function ($query) use ($startDate, $endDate) {
            $query->where(ReportForm3::THANG_BAO_CAO, '>=', strtotime($startDate));
            $query->where(ReportForm3::THANG_BAO_CAO, '<=', strtotime($endDate));
        })->get();
        $result = [
    ...
    public function search($conditions) {
        ...
        if(!empty($conditions['contract_code'])) {
            $searchArr[] = [ReportForm3::MA_PHIEU_GHI, '=', trim($conditions['contract_code'])];
        }
        if(!empty($conditions['contract_disbursement'])) {
            $searchArr[] = [ReportForm3::MA_HOP_DONG, '=', trim($conditions['contract_disbursement'])];
        }

        if(!empty($conditions['store_id'])) {
            $searchArr[] = [ReportForm3::STORE_ID, '=', trim($conditions['store_id'])];
        }
    ...
    public function importNgayDungTinhLai($value) {
        $update = [
            ReportForm3::FLAG_DUNG_TINH_LAI => !empty($value['flag_dung_tinh_lai']) ? $value['flag_dung_tinh_lai'] : "",
            ReportForm3::NGAY_DUNG_TINH_LAI => !empty($value['ngay_dung_tinh_lai']) ? (date('d/m/Y', strtotime($value['ngay_dung_tinh_lai']))) : ""
        ];
        $result = $this->rpModel::where(ReportForm3::MA_PHIEU_GHI, '00000' . (int)$value['ma_phieu_ghi'])->update($update);
    ...
    */
    public function wlog($maPhieuGhi, $action, $createdBy) {
        ...
        $updateReport = $this->rpModel::where(ReportForm3::MA_PHIEU_GHI, $maPhieuGhi)
            ->push(ReportForm3::COLUMN_LOGS, $log);
    }

...
<div id="loading" class="theloading" style="display: none;">
    ...
    @endif
    <legend class="col-md-6" style="padding-bottom: 10px; border-bottom: 1px solid #dee2e6;margin-bottom: 50px;margin-top: 30px;color: #009535;">Phản Hồi Phiếu Ghi Sự Việc</legend>
        <div class="new_report">
        <form action='{{url("cpanel/reportsKsnb/userFeedback/$detail->_id")}}' method="post" enctype="multipart/form-data" id="user_feedback">
          <div class="row" style="padding-top: 10px">
              <div class="col-md-6">
                <legend style="color: #2dbbff; font-size: 20px;">Thông Tin Phiếu Ghi Nhận Sự Việc</legend>
                <div class="mb-3">
                  <label  class="form-label">Tên nhân viên&nbsp;<span class="text-danger">*</span></label>
                  <input type="text" name="user_name_note" class="form-control" id="user_name_note" disabled value='{{implode(", ", $detail["name_note"])}}'>
                </div>
                <div class="mb-3">
                  <label  class="form-label">Email&nbsp;<span class="text-danger">*</span></label>
                  <input type="text" name="user_email_note" class="form-control" id="user_email_note" disabled value='{{implode(", ", $detail["email_note"])}}'>
                </div>
                <div class="mb-3">
                  <label  class="form-label">Tiêu đề sự việc<span class="text-danger">*</span></label>
                  <input type="text" name="title" class="form-control" id="title" disabled value="{{$detail['title']}}">
                </div>
                <div class="mb-3">
                  <label  class="form-label">Nội dung ghi nhận&nbsp;<span class="text-danger">*</span></label>
                  <textarea type="text" name="content" class="form-control" id="content" disabled>{{$detail['content']}}</textarea>
                </div>
                <div class="mb-3">
            ...
            <div class="modal-footer">
              @if(($detail->process == 2 || $detail->process == 9))
                @if(in_array($user, $detail->email_note))
                    <button type="submit" class="btn btn-success" id="userfeedback">Phản hồi</button>
                @endif
              @endif

              <a href='{{url("/cpanel/reportsKsnb/getAllNote")}}' class="btn btn-success" style="margin-right: 50px"> Quay lại</a>
            </div>
            </form>
        </div>
    </div>
    ...
    <div class="modal fade" id="modal_user_feedback" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true" style="z-index: 1000;">
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <p class="msg_success text-primary">Phản hồi từ phía nhân viên đã được gửi cho kiểm soát nội bộ</p>
          </div>
          <div class="modal-footer">

    ...
    <section class="main-content">
        ...
        <input type="hidden" class="form-control" name="_token" value="{{ csrf_token() }}">
            <div class="col-md-2 col-sm-12">
                <label for="store_id" class="col-form-label">TÊN CHIẾN DỊCH<span class="text-danger">*</span></label>
                <input disabled type="text" name="campaign_name" id="campaign_name" class="form-control" value="{{$detail['campaign_name']}}">
            </div>
            <div class="col-md-2 col-sm-12">
                <label for="store_id" class="col-form-label">TÊN VÙNG&nbsp;<span class="text-danger">*</span></label>
                <select disabled is-require class="form-control" name="code_area" id="code_area">
                    <option value="" >--Chọn Vùng--</option>
                      @foreach($code_area as $i)
                          <option value="{{$i['code']}}"@if ($detail['code_area'] == $i['code']) selected @endif>{{$i['title']}}</option>
                      @endforeach
                </select>
            </div>
            <div class="col-md-2 col-sm-12">
                <label for="store_id" class="col-form-label">PHÒNG GIAO DỊCH&nbsp;<span class="text-danger">*</span></label>
                <select class="form-control store" name="store_id[]" id="store_id" multiple="multiple">
                  @foreach ($detail['stores'] as $i)
                    @php $id[] = $i['id'] @endphp
                  @endforeach
                  @foreach ($stores as $store)
                      <option value="{{$store['id']}}" @if(in_array($store['id'], $id)) selected @endif >{{$store['name']}}</option>
                  @endforeach
                </select>
            </div>
            <div class="col-md-2 col-sm-12">
                <label for="store_id" class="col-form-label">SỐ LƯỢT TIẾP CẬN</label>
                <input style="width:100%" type="text" min="0" class="form-control" id="hits" name="hits" value="{{number_format($detail['hits'])}}" oninput="validity.valid||(value='')" onkeydown="return event.keyCode !== 69">
            </div>
        </div>
        <div class="row" style="margin-left: 10px">
            <div class="col-md-2 col-sm-12" style="padding-top: 5px">
                <label for="social" class="form-label">SOCIAL MEDIA&nbsp;</label>
                <input  style="width:100%" type="text" min="0" class="form-control" id="social_media" name="social_media" value="{{number_format($detail['social_media'])}}" oninput="validity.valid||(value='')" onkeydown="return event.keyCode !== 69">
            </div>
            <div class="col-md-2 col-sm-12" style="padding-top: 5px">
                <label for="pr" class="form-label">PR, BÁO CHÍ, TV&nbsp;</label>
                <input  style="width:100%" type="text" min="0" class="form-control" id="pr_tv" name="pr_tv" value="{{number_format($detail['pr_tv'])}}" oninput="validity.valid||(value='')" onkeydown="return event.keyCode !== 69">
            </div>
            <div class="col-md-2 col-sm-12" style="padding-top: 5px">
                <label for="kol" class="form-label">KOL/KOC&nbsp;</label>
                <input  style="width:100%" type="text" min="0" class="form-control" id="kol_koc" name="kol_koc" value="{{number_format($detail['kol_koc'])}}" oninput="validity.valid||(value='')" onkeydown="return event.keyCode !== 69">
            </div>
            <div class="col-md-2 col-sm-12" style="padding-top: 5px">
                <label for="ooh" class="form-label">OOH&nbsp;</label>
                <input  style="width:100%" type="text" min="0" class="form-control" id="ooh" name="ooh" value="{{number_format($detail['ooh'])}}" oninput="validity.valid||(value='')" onkeydown="return event.keyCode !== 69">
            </div>
            <div class="col-md-2 col-sm-12" style="padding-top: 5px">
                <label for="other" class="form-label">KHÁC&nbsp;</label>
                <input  style="width:100%" type="text" min="0" class="form-control" id="other" name="other" value="{{number_format($detail['other'])}}" oninput="validity.valid||(value='')" onkeydown="return event.keyCode !== 69">
            </div>
        ...
        <div style="display:block;margin-top:30px;margin-left: 6px">
            <div style="margin-top:10px;">
                <button style="width:200px;margin-left:10px" id="typeUpdate" type="submit" class="btn btn-success">Lưu thông tin</button>
            ...
            $('#typeUpdate').click(function (event) {
                ...
                if (campaign_name == "") {
                  ...
                  $.ajax({
                    ...
                    url: '{{route("viewcpanel::macom.cost.update", ["id" => $detail["_id"]])}}',
                    type: 'POST',
                    data: formData,
                    processData: false,
                    contentType: false,
                    success: function(data) {
                        ...
                        if (data['status'] == 200) {
                          ...
                          setTimeout(function(){
                            window.location.assign("{{route('viewcpanel::macom.cost.history')}}");
                          }, 2500);
                        }
                    },
...
<script type="text/javascript">
  ...
  $.each(imgs, function (key, value) {
    if (value.file_type == 'image/jpeg' || value.file_type == 'image/png' || value.file_type == 'image/jpg') {
      let block = `
        <div class="block">

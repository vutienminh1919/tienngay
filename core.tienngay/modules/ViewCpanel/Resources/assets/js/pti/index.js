...
$( document ).ready(function() {
        ...
        this.createEL = function ($data, $no, $type = false, $tableId = "clone-object") {
            ...
            $.each(_columns, function( index, value ) {
                ...
                if ($(_columns[index]).attr("format-number")) {
                    ...
                    let val = window[$(_columns[index]).attr("func")](_value, $(_columns[index]));
        ...
        this.exportExcel = function (event) {
            ...
            for (let i = 0; i < this.objJson.length; i++) {
                if (this.objJson[i][1].selected) {
                    ...
                    count++;
                    if (tcvStores.includes(this.objJson[i][1].store.id)) {
                        _rowsTCV += '<tr>' + this.createEL(this.objJson[i][1], countTCV, true, _tableClone) + '</tr>';
                        countTCV++;
                    } else if (tcvDbStores.includes(this.objJson[i][1].store.id)) {
                        _rowsTCVDB += '<tr>' + this.createEL(this.objJson[i][1], countTCVDB, true, _tableClone) + '</tr>';
                        countTCVDB++;
                    } else if (tcvHcmStores.includes(this.objJson[i][1].store.id)) {
    ...
    $("#search-input").on("change", function (event) {
        event.preventDefault();
        var _searchValue = removeDiacritics($("#search-input").val()).toLowerCase();
        ...
        if (!_searchValue) {
            ...
            transactions.forEach(function(value, index) {
                let valueJSON = removeDiacritics(JSON.stringify(value)).toLowerCase();

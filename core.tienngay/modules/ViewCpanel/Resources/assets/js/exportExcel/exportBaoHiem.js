    ...
    function DataList() {
        ...
        this.createEL = function ($data, $no, $type = false, $tableId = "clone-object") {
            ...
            $.each(_columns, function( index, value ) {
                ...
                let _value = null;
                if (splitArr.length > 1 && typeof $data === 'object' && $data !== null) {
                    _value = $data;
                ...
                if ($(_columns[index]).attr("format-number")) {
                    ...
                    window[$(_columns[index]).attr("func")](_value, $(_columns[index]));
        ...
        this.exportExcel = function (event) {
            ...
            for (let i = 0; i < this.objJson.length; i++) {
                let storeId = "";
                if (this.objJson[i][1].hasOwnProperty('store') && this.objJson[i][1]['store'] !== null) {
                    storeId = this.objJson[i][1]['store']['id'];
                } else {
                    this.objJson[i][1]['store'] = this.objJson[i][1]['contract_info']['store'];
                    storeId = this.objJson[i][1]['contract_info']['store']['id'];
                }
                _rows += '<tr>' + this.createEL(this.objJson[i][1], count, true, _tableClone) + '</tr>';
                count++;
                if (tcvStores.includes(storeId)) {
                    _rowsTCV += '<tr>' + this.createEL(this.objJson[i][1], countTCV, true, _tableClone) + '</tr>';
                    countTCV++;
                } else if (tcvDbStores.includes(storeId)) {
                    _rowsTCVDB += '<tr>' + this.createEL(this.objJson[i][1], countTCVDB, true, _tableClone) + '</tr>';
                    countTCVDB++;
                } else if (tcvHcmStores.includes(storeId)) {
                    _rowsTCVHCM += '<tr>' + this.createEL(this.objJson[i][1], countTCVHCM, true, _tableClone) + '</tr>';
                    countTCVHCM++;
                }
            }
            ...
            let _tableExportTCV = document.getElementById(_tableClone).cloneNode(true);
            _tableExportTCV.querySelector('th:last-child').remove();
            _tableExportTCV.querySelector("#table-rows").innerHTML = _rowsTCV;

            let _tableExportTCVDB = document.getElementById(_tableClone).cloneNode(true);
            _tableExportTCVDB.querySelector('th:last-child').remove();
            _tableExportTCVDB.querySelector("#table-rows").innerHTML = _rowsTCVDB;

            let _tableExportTCVHCM = document.getElementById(_tableClone).cloneNode(true);
            _tableExportTCVHCM.querySelector('th:last-child').remove();
            _tableExportTCVHCM.querySelector("#table-rows").innerHTML = _rowsTCVHCM;

            var all = XLSX.utils.table_to_sheet(_tableExport);
            var tcv = XLSX.utils.table_to_sheet(_tableExportTCV);
            var tcvdb = XLSX.utils.table_to_sheet(_tableExportTCVDB);
            var tcvhcm = XLSX.utils.table_to_sheet(_tableExportTCVHCM);

            var wb = XLSX.utils.book_new();
            XLSX.utils.book_append_sheet(wb, all, "Tất cả");
            XLSX.utils.book_append_sheet(wb, tcv, "TCV");
            XLSX.utils.book_append_sheet(wb, tcvdb, "TCV_DB");
            XLSX.utils.book_append_sheet(wb, tcvhcm, "TCV_HCM");

		...
		<div class="col-xs-12">
			<div class="page-title">
				<div class="title_left">
					<h3>Import cập nhật mã phiếu ghi và mã hợp đồng cho phiếu thu
						<br>
						<small>
							<a href="<?php echo base_url() ?>"><i class="fa fa-home"></i> Home</a>/ <a href="#">Import
								cập nhật mã phiếu ghi và mã hợp đồng cho phiếu thu</a></small>
					</h3>
				</div>
			</div>
		</div>
				...
				<div class="row">
					<div class="col-xs-12 col-md-6">
						<div class="dashboarditem_line2 blue">
							<div class="thetitle">
								<i class="fa fa-upload"></i> Import cập nhật mã phiếu ghi và mã hợp đồng cho phiếu thu
							</div>
							<div class="panel panel-default">

								<strong><?= $this->lang->line('Upload') ?>&nbsp;</strong>
								<div class="form-group">
									<input type="file" name="upload_file_transaction" class="form-control" placeholder="sothing">
								</div>
								<a class="btn btn-primary" id="import_transaction"
								   style="margin:0"><?= $this->lang->line('Upload') ?></a>

							</div>
							<a target="_blank" href="<?php echo base_url('importDatabase/list_update_contract_phieu_thu') ?>"> Danh sách phiếu thu đã cập nhật mã hợp đồng</a>
						</div>
					</div>
...
<script>
	$("#import_transaction").click(function (event) {
		...
		$.ajax({
			...
			url: _url.base_url + 'importDatabase/import_update_contract_phieu_thu',
			type: "POST",
			data: formData,
			dataType: 'json',
			processData: false,

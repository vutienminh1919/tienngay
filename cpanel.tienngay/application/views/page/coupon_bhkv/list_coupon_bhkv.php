    ...
    <div class="col-xs-12">
      <div class="page-title">
        <div class="title_left">
          <h3>Danh sách giảm trừ BH KV
            <br>
            <small>
                <a href="<?php echo base_url()?>"><i class="fa fa-home" ></i> Home</a> / <a href="#">Danh sách giảm trừ BH KV</a>
            </small>
          </h3>
        </div>
        <div class="title_right text-right">
          <a href="<?php echo base_url("coupon_bhkv/createCoupon_bhkv")?>" class="btn btn-info " ><i class="fa fa-plus" aria-hidden="true"></i>Tạo giảm trừ BH KV</a>
        </div>
      </div>
    </div>
            ...
            <div class="col-xs-12">
              ...
              <div class="table-responsive">
                <table id="datatable-buttons" class="table table-striped">
                  ...
                  <tbody>
                  <?php 
                    if(!empty($coupon_bhkvData)) {
                        $stt = 0;
                        foreach($coupon_bhkvData as $key => $coupon){
                    ...
                    <tr class='coupon_<?= !empty($coupon->_id->{'$oid'}) ? $coupon->_id->{'$oid'} : "" ?>'>
                      ...
                      <td><?= !empty($coupon->code) ?  $coupon->code : ""?></td>
                        ...
                        <td><?= !empty($coupon->start_date) ? date('m/d/Y', $coupon->start_date ) : ""?></td>
                      <td><?= !empty($coupon->end_date) ? date('m/d/Y', $coupon->end_date) : ""?></td>
                      <td><?= !empty($coupon->updated_at) ?   date('m/d/Y H:i:s', $coupon->updated_at): ""?></td>
                       <td><?= (!empty($coupon->type_coupon) && $coupon->type_coupon==2) ?  'Chỉ CEO duyệt' : "Áp dụng tất cả"?></td>
                      <td>
                      ...
                      <td>
						  <a class="btn btn-primary"  href="<?php echo base_url("coupon_bhkv/update?id=").$coupon->_id->{'$oid'}?>">
...
<!-- /page content -->
<script src="<?php echo base_url();?>assets/js/cai_dat_bhkv/index.js"></script>
    ...
    function set_switchery() {
        $(".aiz_switchery").each(function () {
            ...
            changeCheckbox.onchange = function () {
                $.ajax({url: _url.base_url +'coupon_bhkv/doUpdateStatusCoupon_bhkv?id='+id+'&status='+ changeCheckbox.checked,

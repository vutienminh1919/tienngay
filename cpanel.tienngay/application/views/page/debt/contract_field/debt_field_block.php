<div class="row">
	...
	<?php if (in_array('tbp-thu-hoi-no', $groupRoles) || in_array('supper-admin', $groupRoles) || in_array('van-hanh', $groupRoles)) {
		?>
		<div class="col-lg-2">
			<select class="form-control" id="choose_status_caller">
				<option value="">-- Chọn chức năng chuyển lại Call --</option>
				<option value="1">Gán hợp đồng cho Call</option>
			</select>
		</div>
		<div class="col-lg-2">
			<select class="form-control" id="choose_email_caller">
				<option value="">-- Chọn nhân viên Call --</option>
				<?php if (!empty($debt_caller_emails)) :
					foreach ($debt_caller_emails as $email_caller) : ?>
						<option value="<?= !empty($email_caller) ? $email_caller : ''; ?>"><?= !empty($email_caller) ? $email_caller : ''; ?></option>
					<?php endforeach; ?>
				<?php endif; ?>
			</select>
		</div>
		<div class="col-lg-5">
			<textarea id="approve_note_caller" class="form-control note" rows="1" cols="2"
					  placeholder="Nhập lý do"></textarea>
		</div>
		<div class="col-lg-1">
			<a class="btn btn-info"
			   onclick="assigned_contract_back_to_call(this)" data-tab="1">
				Xác nhận
			</a>
		</div>
	<?php } ?>
...
<div class="table-responsive">
	...
	<table class="table table-bordered m-table table-hover table-calendar table-report datatablebuttoncaller">
				...
				<?php if ($debt_field_email == $userInfo['email'] || in_array('supper-admin', $groupRoles) || in_array("tbp-thu-hoi-no", $groupRoles) || in_array("van-hanh", $groupRoles)) { ?>
					<tr role="row" class="even parent">
						...
						<?php if ($userSession['is_superadmin'] == 1 || in_array('tbp-thu-hoi-no', $groupRoles) || in_array('van-hanh', $groupRoles)) { ?>
							<td>
								<input type="checkbox"
									   name="date-call_to_field[]"
									   value="<?php echo $contract->code_contract; ?>"
									   class="checkbox_approve_caller">
							</td>
						<?php } ?>
						<td>
							<div class="dropdown">
						...
						<?php if (!empty($contract->debt_field_email)): ?>
							<td style="text-align: center" class="<?php if ($contract->evaluate == 1 || $contract->evaluate == 4) : ?>
								...
								<br>
								<a style="text-align: center" href="javascript:void(0)" onclick="showLogDebt('<?= $contract->contract_id ?>')"

			...
			foreach ($contractData as $key => $contract) {
				...
				<?php if ($debt_caller_email == $userInfo['email'] || in_array('supper-admin', $groupRoles) || in_array("tbp-thu-hoi-no", $groupRoles) || in_array("van-hanh", $groupRoles) || in_array("lead-thn", $groupRoles)) { ?>
					<tr role="row" class="even parent">
						...
						<!--													<td>-->
						<!--														--><?php //if (in_array($contract->bucket, ['B0', 'B1']) && !isset($contract->time_range_to_field) && !in_array($contract->status,[37, 277, 278, 279])) { ?>
						<!--															<input type="checkbox"-->
						<!--																   name="date-call[]"-->
						<!--																   value="--><?php //echo $contract->_id->{'$oid'}; ?><!--"-->
						<!--																   class="checkbox_date_to_field">-->
						<!--														--><?php //} ?>
						<!--													</td>-->
						<td>
							<div class="dropdown">
						...
						<td style="text-align: center"><?= !empty($contract->code_contract) ? $contract->code_contract : "" ?><br>

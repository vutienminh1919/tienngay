<div class="row">
	...
	<?php if (in_array('tbp-thu-hoi-no', $groupRoles) || in_array('supper-admin', $groupRoles) || in_array('van-hanh', $groupRoles)) {
		?>
		<div class="col-lg-2">
			<select class="form-control" id="choose_status_field">
				<option value="">-- Chọn chức năng chuyển Field --</option>
				<option value="1">Gán hợp đồng cho Field</option>
			</select>
		</div>
		<div class="col-lg-2">
			<select class="form-control" id="choose_email_field">
				<option value="">-- Chọn nhân viên Field --</option>
				<?php if (!empty($debt_field_emails)) :
					foreach ($debt_field_emails as $email_field) : ?>
						<option value="<?= !empty($email_field) ? $email_field : ''; ?>"><?= !empty($email_field) ? $email_field : ''; ?></option>
					<?php endforeach; ?>
				<?php endif; ?>
			</select>
		</div>
		<div class="col-lg-5">
			<textarea id="approve_note_field" class="form-control note" rows="1" cols="2"
					  placeholder="Nhập lý do"></textarea>
		</div>
		<div class="col-lg-1">
			<a class="btn btn-info"
			   onclick="assigned_contract_to_field(this)" data-tab="1">
				Xác nhận
			</a>
		</div>
	<?php } ?>
...
<div class="table-responsive">
	...
	<table class="table table-bordered m-table table-hover table-calendar table-report datatablebuttonfield">
			...
			foreach ($contractData as $key => $contract) {
				...
				<?php if ($debt_caller_email == $userInfo['email'] || in_array('supper-admin', $groupRoles) || in_array("tbp-thu-hoi-no", $groupRoles) || in_array("van-hanh", $groupRoles) || in_array("lead-thn", $groupRoles)) { ?>
					<tr role="row" class="even parent">
						...
						<td style="text-align: center"><?= !empty($contract->code_contract) ? $contract->code_contract : "" ?>
							<br>
							<a class="btn btn-info"
							   href="javascript:void(0)"
							   onclick="history_processing('<?= !empty($contract->contract_id) ? $contract->contract_id : "" ?>')">

    ...
    <div class="col-xs-12">
      <div class="page-title">
        <div class="title_left">
          <h3>Chi tiết số liệu KPI giao dịch viên
          <br>
			<small>
			<a href="<?php echo base_url()?>"><i class="fa fa-home" ></i> Home</a> / <a href="#>">Chi tiết số liệu KPI giao dịch viên</a>
			</small>
          </h3>
        </div>
      </div>
        ...
        <div class="x_title">
          ...
          <div class="row">
            <div class="col-xs-12 col-lg-12">
				  ...
				  <form action="<?php echo base_url('kpi/listDetailKPI_user')?>" method="get" style="width: 100%;">
				  	      <div class="row">
					  <div class="col-lg-3">
						
						  <div class="input-group">
							  <span class="input-group-addon">Tháng</span>
							  <input type="month" name="fdate" class="form-control" value="<?= !empty($fdate) ?  $fdate : ""?>" >
						  </div>
					  </div>
					  <!-- 
					  <div class="col-lg-3">
					   ...
					   <div class="col-lg-2">
						 <!--  <label>Email nhân viên</label> -->
						  <input type="text" name="customer_email" class="form-control" placeholder="Email nhân viên" value="<?php echo $customer_email; ?>">
					  </div>
                  ...
                  <tbody>
                    ...
                    ?>
                      ...
                      <tr>
						...
						<td><?= !empty($kpi->user_email) ? $kpi->user_email : '' ?></td>
						<td><?= !empty($kpi->month) ? $kpi->month.'/'.$kpi->year : '' ?></td>
						
						<td class="center">
			          	<b class="text-danger">KPI: <?= ($kpi->kpi->giai_ngan_CT>0) ?  round(($kpi->sum_giai_ngan/$kpi->kpi->giai_ngan_CT)*($kpi->kpi->giai_ngan_TT)) : 0 ?> %</b><br>
			          	Đạt được: <?= !empty($kpi->sum_giai_ngan) ? number_format($kpi->sum_giai_ngan) : '0' ?>đ
						<br/>Chỉ tiêu: <?= !empty($kpi->kpi->giai_ngan_CT) ? number_format($kpi->kpi->giai_ngan_CT) : '0' ?>đ
						<br/>Chỉ trọng: <?= !empty($kpi->kpi->giai_ngan_TT) ? $kpi->kpi->giai_ngan_TT : '0' ?>%
					
					    </td>
						<td class="center">
							<b class="text-danger">KPI: <?= ($kpi->kpi->bao_hiem_CT>0) ? round(($kpi->sum_bao_hiem/$kpi->kpi->bao_hiem_CT)*($kpi->kpi->bao_hiem_TT)) : 0 ?> %</b><br>
			          	Đạt được: <?= !empty($kpi->sum_bao_hiem) ? number_format($kpi->sum_bao_hiem) : '0' ?>đ
						<br/>Chỉ tiêu: <?= !empty($kpi->kpi->bao_hiem_CT) ? number_format($kpi->kpi->bao_hiem_CT) : '0' ?>đ
						<br/>Chỉ trọng: <?= !empty($kpi->kpi->bao_hiem_TT) ? $kpi->kpi->bao_hiem_TT : '0' ?>%
							
						</td>
						<td class="center">
							<b class="text-danger">KPI: <?= ($kpi->kpi->khach_hang_moi_CT>0) ? round(($kpi->count_khach_hang_moi/$kpi->kpi->khach_hang_moi_CT)*($kpi->kpi->khach_hang_moi_TT)) : 0 ?> %</b><br>
			          	Đạt được: <?= !empty($kpi->count_khach_hang_moi) ? $kpi->count_khach_hang_moi : '0' ?>
						<br/>Chỉ tiêu: <?= !empty($kpi->kpi->khach_hang_moi_CT) ? $kpi->kpi->khach_hang_moi_CT : '0' ?>
						<br/>Chỉ trọng: <?= !empty($kpi->kpi->khach_hang_moi_TT) ? $kpi->kpi->khach_hang_moi_TT : '0' ?>%
                     </td>
                      </tr>

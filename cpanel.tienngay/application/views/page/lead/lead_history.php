...
<div class="right_col" role="main">
	...
	$email_nv = !empty($_GET['email_nv']) ? $_GET['email_nv'] : "";

	$user_name = !empty($_GET['user_name']) ? $_GET['user_name'] : "";
	$phone_name = !empty($_GET['phone_name']) ? $_GET['phone_name'] : "";
	...
	<div class="row top_tiles">
					...
					<h3>Lịch sử cuộc gọi
						...
						<small>
							<a href="<?php echo base_url() ?>"><i class="fa fa-home"></i> Home</a>/ <a
									href="<?php echo base_url('lead_custom') ?>">Quản lý lead</a> / <a href="#">Lịch sử
								cuộc gọi</a>
						</small>
					</h3>
		...
		<div class="col-xs-12">
			<div class="row">
				<form action="<?php echo base_url('lead_custom/historyCall') ?>" method="get" style="width: 100%;">
					<div class="col-lg-3">
						<label></label>
					...
					<!-- <div class="col-lg-2">
						...
						<div class="input-group w-100">
							<input type="text" name="sdt" class="form-control" value="<?= !empty($sdt) ? $sdt : "" ?>"
					...
					<div class="col-lg-2">
						...
						<div class="input-group w-100">
							...
							<input type="text" name="email_nv" class="form-control" value="<?= !empty($email_nv) ? $email_nv : "" ?>"
					...
					<div class="col-lg-2">
						...
						<div class="input-group w-100">
							...
							<input type="text" name="user_name" class="form-control" value="<?= !empty($user_name) ? $user_name : "" ?>"
					...
					<?php if (in_array('quan-ly-khu-vuc',$groupRoles)): ?>
						<div class="col-lg-2">
							...
							<div class="input-group w-100">
								<input type="text" name="phone_name" class="form-control" value="<?= !empty($phone_name) ? $phone_name : "" ?>"
					...
					if (!empty($recordingData)) {
						...
						foreach ($recordingData as $key => $history) {
							...
							<tr id="<?= $history->_id->{'$oid'} ?>">
								...
								<td><?= ($history->fromNumber) ? hide_phone($history->fromNumber) : ''; ?>

									<br>
									<?= ($history->fromUser) ? 'Nhánh: ' . $history->fromUser->ext : ''; ?>
								</td>
								<td><?= ($history->toNumber) ? hide_phone($history->toNumber) : ''; ?>
								...
								<td>Bắt
									...
									Trả
									lời:  
										<?= (!empty($history->answerTime) && (int)($history->answerTime) > 0) ? date("d/m/Y H:i:s", $history->answerTime / 1000) : "Không có";
									?>
									<br>
									Kết
								...
								<td>Tổng time: <?= ($history->duration) ? $history->duration : 0 ?><br>
									Tổng time tư vấn: <?= ($history->billDuration) ? $history->billDuration : 0 ?><br>
								</td>
								<td class="text-right">
	...
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			...
			<div class="modal-body">
				...
				<?php if (in_array('telesales',$groupRoles) || in_array('thu-hoi-no',$groupRoles)): ?>
					<audio controls class="w-100" id="player" >
						<source src="" type="audio/mp3" id="audio" >
					</audio>
				<?php else: ?>
					<audio controls class="w-100" id="player" controlsList="nodownload" >
						<source src="" type="audio/mp3" id="audio" >
					</audio>
				<?php endif; ?>

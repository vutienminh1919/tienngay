...
<div class="row">
	<?php if (in_array("tbp-cskh", $groupRoles)): ?>
		<div class="col-lg-2 text-right">
			...
			target="_blank"
			href="<?= base_url() ?>excel/exportLeadPGDReturn?fdate=<?= $fdate . '&tdate=' . $tdate . '&fullname=' .$fullname . '&source_s=' .$source. "&cskh=" .$cskh. '&status_sale_1='.$status_sale. '&priority=' .$priority. '&sdt=' .$sdt?>"
		...
		<div class="dropdown-menu">
			...
			<a class="dropdown-item">
				<div class="custom-control custom-checkbox">
					...
					<label class="custom-control-label" >LÝ DO TRẢ VỀ</label>
				</div>
			</a>
			<a class="dropdown-item">
				<div class="custom-control custom-checkbox">
					<input type="checkbox" class="custom-control-input" data-control-column="12" checked="checked">
					<label class="custom-control-label" >PGD CHUYỂN VỀ</label>
				</div>
			</a>
			<a class="dropdown-item" href="#">
				<div class="custom-control custom-checkbox">
...
<div class="table-responsive">
	<div><?php echo $result_count10; ?></div>
	<table class="table table-striped thedatatabl hide-show-column">
		...
		<tr>
			...
			<th class="hide-show-reason-cancel">LÝ DO TRẢ VỀ</th>
			<th class="hide-show-store">PGD CHUYỂN VỀ</th>
			<th class="hide-show-store">PGD LÝ DO CHUYỂN VỀ</th>
			<th>SẢN PHẨM VAY</th>
			...
			foreach ($leadsData10 as $key => $lead) {
             ...
             <a class="btn btn-info "
               ...
               Nhận
					...
					<td class="hide-show-lead-status-all"><?= !empty($lead->status_pgd) ? status_pgd((int)$lead->status_pgd, false) : status_pgd(0) ?></td>
					<td><?= !empty($lead->reason_return) ? reason_return($lead->reason_return) : '' ?></td>
					<td class="hide-show-reason-cancel"><?= !empty($lead->pgd_note) ? wordwrap($lead->pgd_note, 50, "<br>\n") : "" ?></td>

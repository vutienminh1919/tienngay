		...
		<div class="col-xs-12">
			<div class="page-title">
				<div class="title_left">
					<h3><?php echo $this->lang->line('news_list') ?>
						<br>
						<small>
							<a href="<?php echo base_url() ?>"><i class="fa fa-home"></i> Home</a> / <a
									href="#"><?php echo $this->lang->line('news_list') ?></a>
						</small>
					</h3>
				</div>
				<div class="title_right text-right">
					<a href="<?php echo base_url("news/createnews") ?>" class="btn btn-info "><i class="fa fa-plus"
																								 aria-hidden="true"></i> <?php echo $this->lang->line('create_news') ?>
					</a>
				</div>
			</div>
		</div>
						...
						<div class="col-xs-12">
							...
							<div class="table-responsive">
										...
										foreach ($newsData as $key => $news) {
											if ($news->status != 'block') {
												...
												<tr class='news_<?= !empty($news->_id->{'$oid'}) ? $news->_id->{'$oid'} : "" ?>'>
													<td><?php echo $stt ?></td>
													<td class="w-25">
														<img src="<?= !empty($news->image) ? $news->image : base_url() . "assets/imgs/default_image.png"; ?>"
															 class="img-fluid img-thumbnail" alt="NULL">
													</td>

													<td><?= !empty($news->title_vi) ? $news->title_vi : "" ?></td>
													<td><?= !empty($news->level) ? $news->level : "" ?></td>
													<td><?= !empty($news->updated_at) ? date('d/m/Y H:i:s', $news->updated_at) : "" ?></td>
													<td>
													...
													<td>
														...
														<?php
														$type = !empty($news->type) ? $news->type : "";
														if ($type == '1') {
															echo 'Tin tức';
														} elseif ($type == '2') {
															echo 'Tuyển dụng';
														} elseif (isset($news->name_type_new)) {
															echo $news->name_type_new;
														} else {
															echo '';
														}
														?>
													...
													<td>
														<a class="btn btn-primary"
														   href="<?php echo base_url("news/update?id=") . $news->_id->{'$oid'} ?>">
							  ...
							  <i class="fa fa-close"></i> Xóa
													...
													<!-- Modal HTML -->
													<div id="detele_<?php echo $news->_id->{'$oid'} ?>"
														 class="modal fade">
		...
		function set_switchery() {
			$(".aiz_switchery").each(function () {
				...
				changeCheckbox.onchange = function () {
					$.ajax({
						url: _url.base_url + 'news/doUpdateStatusNews?id=' + id + '&status=' + changeCheckbox.checked,
						method: "GET",
						dataType: 'json',

		...
		<div class="col-xs-12">
			<div class="page-title">
				<div class="title_left">
					<h3>THỐNG KÊ TIỀN THU THEO CHI NHÁNH</h3>
				</div>
			</div>
		</div>
					...
					<div class="table-responsive">
						<table id="summary-total"
							...
							<tr style="background: #8DEEEE; font-weight: bold">
								...
								<td><?= !empty($sum_cod_in_day) ? number_format($sum_cod_in_day) . " đ" : 0 ?></td>
								<td><?= !empty($sum_cod_out_day) ? number_format($sum_cod_out_day) . " đ" : 0 ?></td>
								<td><?= !empty($sum_of_day) ? number_format($sum_of_day) . " đ" : 0 ?></td>
							</tr>
							<?php
							if (!empty($total_parent)) {
								...
								foreach ($total_parent as $key => $amount_parent) {
									...
									<tr>
										...
										<td style="font-weight: bold"><?= !empty($amount_parent->total_cod_not_yet_send_day) ? number_format($amount_parent->total_cod_not_yet_send_day) . " đ" : 0 ?></td>
										<td style="font-weight: bold"><?= !empty($amount_parent->total_cod_in_day) ? number_format($amount_parent->total_cod_in_day) . " đ" : 0 ?></td>
										<td style="font-weight: bold"><?= !empty($amount_parent->total_cod_out_day) ? number_format($amount_parent->total_cod_out_day) . " đ" : 0 ?></td>
										<td style="font-weight: bold"><?= !empty($amount_parent->total_cod_day) ? number_format($amount_parent->total_cod_day) . " đ" : 0 ?></td>
									</tr>
									<?php
								}
							} ?>
...
<!--									<a href="--><?//= base_url() ?><!--excel/exportCashManagement?--><?//= 'fdate=' . $fdate . '&tdate=' . $tdate . '&code=' . $code . '&total=' . $total . '&store=' . $store . '&type_transaction=' . $type_transaction . '&status=' . $status ?><!--"-->
...
<!--								</button>-->
							...
							<div class="dropdown" style="display:inline-block">
								...
								<ul id="lockdulieu" class="dropdown-menu dropdown-menu-right"
									...
									<div class="row">
										<form action="<?php echo base_url('CashManagement') ?>" method="get"
											  style="width: 100%">
											<div class="col-xs-12 col-md-6">
								...
								foreach ($transactionData as $key => $transaction) {
									...
									<tr style="background: #8DEEEE">
										...
										<td style="font-weight: bold">
											...
											foreach ($transaction as $key1 => $tran) {
												foreach ($tran as $key2 => $t) {
													$total_in_store += $t->total_amount_in_user;
												}
											}
											?>
											<?= number_format($total_in_store) ?>
										</td>
										<td style="font-weight: bold">
											<?php
											$total_out_store = 0;
											foreach ($transaction as $key1 => $tran) {
												foreach ($tran as $key2 => $t) {
													$total_out_store += $t->total_amount_out_user;
												}
											}
											?>
											<?= number_format($total_out_store) ?>
										</td>
										<td style="font-weight: bold">
											<?php
											$total_amount_transaction_store = 0;
											foreach ($transaction as $key1 => $tran) {
												foreach ($tran as $key2 => $t) {
													$total_amount_transaction_store += $t->total_amount_transaction;
												}
											}
											?>
											<?= number_format($total_amount_transaction_store) ?>
										</td>
										<td colspan="6"></td>
									</tr>
									<?php
									...
									foreach ($transaction as $key1 => $trans) {
										...
										<tr style="background: #ede8ab">
											...
											<td style="font-weight: bold">
												<?php
												echo number_format($tran[0]->total_amount_in_user);
												?>
											</td>
											<td style="font-weight: bold">
												<?php
												echo number_format($tran[0]->total_amount_out_user);
												?>
											</td>
											<td style="font-weight: bold">
												<?php
												echo number_format($tran[0]->total_amount_transaction);
												?>
											</td>
											<td colspan="5"></td>
										</tr>
										...
										foreach ($tran as $key2 => $t) {
											...
											<tr>
												...
												<td>
													<?php
													if (!empty($t->approved_at) && $t->status == 1) {
														echo date('d/m/Y <\b\r>H:i:s', $t->approved_at);
													} else {
														echo "";
													}
													?>
												</td>
												<td><?= !empty($t->approved_by) ? $t->approved_by : $t->updated_by ?></td>
												<td><?php
													if ($t->status == 11) {
												...
												<td>
													<?php if (in_array($t->type, [1])) { ?>
														<a href="<?php echo base_url('transaction/view/' . $t->id) ?>"
														   class="dropdown-item"
														   target="_blank"
														   style="color: blue">
															Xem chi tiết
													...
													<?php } elseif (in_array($t->type, [3, 4])) { ?>
														<a href="<?php echo base_url('transaction/viewContract/' . $t->id) ?>"
														   class="dropdown-item"
														   target="_blank"
														   style="color: blue">
															Xem chi tiết
													...
													<?php } elseif ($t->type == 7 && $t->status != 10) { ?>
														<a href="<?php echo base_url('transaction/viewDetailHeyU/' . $t->id) ?>"
														   class="dropdown-item"
														   target="_blank"
														   style="color: blue">
															Xem chi tiết
													...
													<?php } elseif ($t->type == 8 && $t->status != 10) { ?>
														<a href="<?php echo base_url('transaction/viewDetailMicTnds/' . $t->id) ?>"
														   class="dropdown-item"
														   target="_blank"
														   style="color: blue">
															Xem chi tiết
													...
													<?php } elseif ($t->type == 10 && $t->status != 10) { ?>
														<a href="<?php echo base_url('transaction/viewDetailVbiTnds/' . $t->id) ?>"
														   class="dropdown-item"
														   target="_blank"
														   style="color: blue">
															Xem chi tiết
													...
													<?php } elseif ($t->type == 11 && $t->status != 10) { ?>
														<a href="<?php echo base_url('transaction/viewDetailVbiUtv/' . $t->id) ?>"
														   class="dropdown-item"
														   target="_blank"
														   style="color: blue">
															Xem chi tiết
													...
													<?php } elseif ($t->type == 12 && $t->status != 10) { ?>
														<a href="<?php echo base_url('transaction/viewDetailVbiSxh/' . $t->id) ?>"
														   class="dropdown-item"
														   target="_blank"
														   style="color: blue">
															Xem chi tiết

	...
	<div class="row top_tiles">
		<div class="col-xs-12">
			<div class="page-title">
				<div class="row">
					<div class="col-xs-12">
						<h3><?= $this->lang->line('Contract_management') ?> gia hạn
							<br>
							<small>
								<a href="<?php echo base_url() ?>"><i class="fa fa-home"></i> Home</a> / <a
										href="<?php echo base_url() ?>pawn/contract"><?php echo $this->lang->line('Contract_management') ?> gia hạn</a>
							</small>
						</h3>
					</div>
				...
				<div class="x_title">
					<div class="row">
						<div class="col-xs-12">
							...
							<?php } ?>
							<div class="row">
								<form action="<?php echo base_url('pawn/search_gh') ?>" method="get" style="width: 100%;">
									<div class="col-xs-12">
										<div class="row">
											...
											<div class="col-xs-12 col-lg-2">
												...
												<select class="form-control" name="status">
													...
													<?php foreach (contract_status() as $key => $value)   {
														if(!in_array($key,[11,13,21,25,29,30,26,33,41])) continue;
											...
											<div class="col-xs-12 col-lg-2">
												...
												<select class="form-control" name="search_htv">
													...
													<option value="DKX" <?php echo $search_htv == 'DKX' ? 'selected' : '' ?>>Cho vay</option>
													<option value="CC" <?php echo $search_htv == 'CC' ? 'selected' : '' ?>>Cầm cố</option>
													<option value="TC" <?php echo $search_htv == 'TC' ? 'selected' : '' ?>>Tín chấp</option>
												</select>
											</div>
											<div class="col-xs-12 col-lg-2">
												<label>&nbsp;</label>
		...
		if (!empty($contractData)) {
			foreach ($contractData as $key => $contract) {
				...
				if ($contract->loan_infor->type_property->code == "TC") {
					$printed .= '<li><a href="javascript:void(0)" onclick="show_popup_print_contract_mortgage(this)" data-id="' . $contract_id . '" class="dropdown-item"> In hợp đồng</a></li>';
				...
				<tr>
					...
					<?php } ?>
																...
																<?php
																// buttom gửi hội sở duyệt gia hạn
																if ((in_array($contract->status, array(17, 22)) && $contract->loan_infor->type_interest == 2) && $contract->debt->check_gia_han == 1) { ?>
																	<li><a href="javascript:void(0)"
...
<?php } ?> -->
					...
					if (in_array($contract->status, array(21))) { ?>
						<li><a href="javascript:void(0)"
							   onclick="gui_asm_duyet_gia_han(this,false)"
							   data-id="<?= !empty($contract->_id->{'$oid'}) ? $contract->_id->{'$oid'} : '' ?>"
							   class="dropdown-item">
								Gửi ASM duyệt gia hạn
						</a></li>
					<?php } ?>
						<?php
					// buttom duyet gia hạn hợp đồng
					if (in_array($contract->status, array(23))) { ?>
						<li><a href="javascript:void(0)"
							   onclick="gui_asm_duyet_co_cau(this,false)"
							   data-id="<?= !empty($contract->_id->{'$oid'}) ? $contract->_id->{'$oid'} : '' ?>"
							   class="dropdown-item">
								Gửi ASM duyệt cơ cấu
						</a></li>
					<?php } ?>
																...
																if (in_array($contract->status, array(2))
																	...
																	<li><a href="javascript:void(0)"
																		   ...
																		   data-id="<?= !empty($contract->_id->{'$oid'}) ? $contract->_id->{'$oid'} : "" ?>"
																		   class="dropdown-item cht_tu_choi"> CHT Không
																			duyệt</li>
																<?php } ?>
																<?php
																// buttom chuyển lên hội sở
																if (in_array($contract->status, array(2, 8, 36))
																		&& in_array("5dedd2d868a3ff310000364b", $userRoles->role_access_rights)) { ?>
																	<li><a href="javascript:void(0)"
																		   onclick="chuyen_asm_duyet(this)"
																		   data-id="<?= !empty($contract->_id->{'$oid'}) ? $contract->_id->{'$oid'} : "" ?>"
																		   class="dropdown-item chuyen_asm_duyet"> Gửi
																			ASM
																			duyệt
																	</li>
																<?php } ?>
...
<?php } ?> -->
																...
																if (in_array($contract->status, array(17))) { ?>
																	<li>
																		<a href="<?php echo base_url("borrowed/index_borrowed") ?>"
																		   class="dropdown-item">
																			Mượn hồ sơ</a></li>
																<?php } ?>
															<?php } ?>
...
<?php } ?> -->
																...
																if (in_array($contract->status, array(2, 8, 36))) { ?>
																	<li><a href="javascript:void(0)"
																		   onclick="chuyen_asm_duyet(this)"
																		   data-id="<?= !empty($contract->_id->{'$oid'}) ? $contract->_id->{'$oid'} : "" ?>"
																		   class="dropdown-item chuyen_asm_duyet"> CHT
																			Duyệt</li>
																<?php } ?>
																<!-- hội sở -->
																<?php
																...
																if (in_array($contract->status, array(5))) { ?>
																	<li><a href="javascript:void(0)"
																		   onclick="hsduyet(this)"
																		   data-id="<?= !empty($contract->_id->{'$oid'}) ? $contract->_id->{'$oid'} : "" ?>"
																		   class="dropdown-item duyet">Hội sở Duyệt</li>
																<?php } ?>
																<!-- kế toán -->
																<?php
				...
				<td><?=  $ci->contract_model->getMaHopDongVay($contract); ?></td>
					<td><?= Contract_model::getMaPhuLuc($contract); ?></td>
					...
					<td>
						...
						foreach (contract_status() as $key => $value) {
							if($status==$key)
							{
							if (($status == 5) && ($contract->loan_infor->amount_loan < 50000000) && ($contract->loan_infor->type_property->code == "XM") && ($contract->loan_infor->type_loan->code == "CC")) {
							echo "Chờ asm duyệt";
					...
					<td><?php if($contract->status!=19 && $contract->status!=17 )
					{ if(!empty($contract->debt->check_tt_gh) && $contract->debt->check_tt_gh == 1)
					{ echo   "<i class='fa fa-circle text-success' aria-hidden='true'></i> Đủ" ;
				  }else{ echo   "<i class='fa fa-circle text-danger' aria-hidden='true'></i>Thiếu"; }} ?></td>
	...
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			...
			<div class="modal-body">
				...
				<br>
				<div class="row">
					<span class="sample-receipt-one">- Biên bản bàn giao tài sản mẫu (1): áp dụng sau khi khách hàng kí Thỏa thuận ba bên</span> <br>

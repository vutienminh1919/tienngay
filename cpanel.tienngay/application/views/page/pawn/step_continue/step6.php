<div class="x_panel setup-content" id="step-6">
	<div class="x_content">
		...
		<div class="form-group row">
			...
			<div class="col-lg-6 col-sm-12 col-12">
								...
								<table class="table table-striped hide-show-column" id="datatable-button">
									...
									<?php else: ?>
										...
										<tr id="company-<?php echo  $value->_id->{'$oid'} ?>">
											...
											<td><button class="del-company" data-id="<?php echo $value->_id->{'$oid'} ?>"><i class="fa fa-trash" style="color: red" aria-hidden="true"></i></button></td>
											</tr>
										<?php endforeach; ?>
									<?php endif; ?>
									</tbody>
				...
				<div class="title_right text-right">
					<button class="btn btn-info modal_company_update" data-toggle="modal" data-target="#addNewCompanyModal"><i
								class="fa fa-plus" aria-hidden="true"></i> Thêm mới
					</button>
				</div>
			</div>
		</div>
		<input id="customer_phone_number1" value="<?php echo $contractInfor->customer_infor->customer_phone_number ?>" hidden>
		<br>
					...
					<div class="col-md-6">
						<div id="exception1" <?php if (empty($contractInfor->expertise_infor->exception1_value[0])): ?> style="display: none" <?php endif;?>>
							...
							<select id="lead_exception_E1" class="form-control" name="lead_exception_E1[]" multiple="multiple" data-placeholder="Các lý do ngoại lệ E1" >
								...
								?>
								<option value="1" <?=  ( is_array($value1) && in_array("1", $value1)) ? 'selected' : '' ?> >E1.1: Ngoại lệ về tuổi vay</option>
								<option value="2" <?=  ( is_array($value1) && in_array("2", $value1)) ? 'selected' : '' ?> >E1.2: Ngoại lệ về giấy tờ định danh: CMND/CCCD mờ ảnh / mờ số không đủ điều kiện</option>
							</select>
								</div>
								<div class="col-md-2">
						...
						<div id="exception2" <?php if (empty($contractInfor->expertise_infor->exception2_value[0])): ?> style="display: none" <?php endif;?>>
							...
							<select   id="lead_exception_E2" class="form-control" name="lead_exception_E2[]" multiple="multiple" data-placeholder="Các lý do ngoại lệ E2" >
								...
								?>
								<option value="3" <?=  ( is_array($value2) && in_array("3", $value2)) ? 'selected' : '' ?> >E2.1: Khách hàng KT3 tạm trú dưới 6 tháng</option>
								<option value="4" <?=  ( is_array($value2) && in_array("4", $value2)) ? 'selected' : '' ?> >E2.2: Khách hàng KT3 không có hợp đồng thuê nhà, sổ tạm trú, xác minh qua chủ nhà trọ</option>
							</select>
								</div>
								<div class="col-md-2">
						...
						<div id="exception3" <?php if (empty($contractInfor->expertise_infor->exception3_value[0])): ?> style="display: none" <?php endif;?>>
							...
							<select   id="lead_exception_E3" class="form-control" name="lead_exception_E3[]" multiple="multiple" data-placeholder="Các lý do ngoại lệ E3" >
								...
								?>
								<option value="5" <?=  ( is_array($value3) && in_array("5", $value3)) ? 'selected' : '' ?> >E3.1: Khách hàng thiếu một trong những chứng từ chứng minh thu nhập</option>
						...
						<div id="exception4" <?php if (empty($contractInfor->expertise_infor->exception4_value[0])): ?> style="display: none" <?php endif;?>>
							...
							<select   id="lead_exception_E4" class="form-control" name="lead_exception_E4[]" multiple="multiple" data-placeholder="Các lý do ngoại lệ E4" >
								...
								?>
								<option value="6" <?=  ( is_array($value4) && in_array("6", $value4)) ? 'selected' : '' ?> >E4.1: Ngoại lệ về TSĐB khác TSĐB trong quy định về SP hiện hành của công ty (đất, giấy tờ khác...)</option>
								<option value="7" <?=  ( is_array($value4) && in_array("7", $value4)) ? 'selected' : '' ?> >E4.2: Ngoại lệ về lãi suất sản phẩm</option>
						...
						<div id="exception5" <?php if (empty($contractInfor->expertise_infor->exception5_value[0])): ?> style="display: none" <?php endif;?>>
							...
							<select   id="lead_exception_E5" class="form-control" name="lead_exception_E5[]" multiple="multiple" data-placeholder="Các lý do ngoại lệ E5" >
								...
								?>
								<option value="8" <?=  ( is_array($value5) && in_array("8", $value5)) ? 'selected' : '' ?> >E5.1: Ngoại lệ về điều kiện đối với người tham chiếu</option>
								<option value="9" <?=  ( is_array($value5) && in_array("9", $value5)) ? 'selected' : '' ?> >E5.2: Ngoại lệ PGD gọi điện cho tham chiếu không sử dụng hệ thống phonet</option>
						...
						<div id="exception6" <?php if (empty($contractInfor->expertise_infor->exception6_value[0])): ?> style="display: none" <?php endif;?>>
							...
							<select   id="lead_exception_E6" class="form-control" name="lead_exception_E6[]" multiple="multiple" data-placeholder="Các lý do ngoại lệ E6" >
								...
								?>
								<option value="10" <?=  ( is_array($value6) && in_array("10", $value6)) ? 'selected' : '' ?> >E6.1: KH có nhiều hơn 3 KV ở các app hay tổ chức tín dụng, ngân hàng khác</option>
							</select>
								</div>
								<div class="col-md-2">
						...
						<div id="exception7" <?php if (empty($contractInfor->expertise_infor->exception7_value[0])): ?> style="display: none" <?php endif;?>>
							...
							<select   id="lead_exception_E7" class="form-control" name="lead_exception_E7[]" multiple="multiple" data-placeholder="Các lý do ngoại lệ E7" >
								...
								?>
								<option value="11" <?=  ( is_array($value7) && in_array("11", $value7)) ? 'selected' : '' ?> >E7.1: Khách hàng vay lại có lịch sử trả tiền tốt</option>
								<option value="12" <?=  ( is_array($value7) && in_array("12", $value7)) ? 'selected' : '' ?> >E7.2: Thu nhập cao, gốc còn lại tại thời điểm hiện tại thấp</option>
								<option value="13" <?=  ( is_array($value7) && in_array("13", $value7)) ? 'selected' : '' ?> >E7.3: KH làm việc tại các công ty là đối tác chiến lược</option>
								<option value="14" <?=  ( is_array($value7) && in_array("14", $value7)) ? 'selected' : '' ?> >E7.4: Giá trị định giá tài sản cao</option>
							</select>
								</div>
								<div class="col-md-2">
		...
		?>
		<button class="btn btn-primary  pull-right save_contract" type="button" data-toggle="modal"
				data-step="<?php echo $step ?>" data-target="#saveContract5">Lưu lại
		</button>
	...
	<div class="modal-dialog modal-confirm">
		<div class="modal-content">
			...
			<div class="modal-footer">
				...
				<button type="button"
						class="btn btn-success btn-continue-create-contract"><?= $this->lang->line('ok') ?></button>
			</div>
		</div>
	</div>
</div>

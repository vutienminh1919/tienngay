<div class="x_panel setup-content" id="step-5">
	<div class="x_content">
			...
			<div class="col-md-6 col-sm-6 col-xs-12">
				<select class="form-control" id="stores">
					...
					foreach ($stores as $key => $value) {
						if (in_array($value->_id->{'$oid'}, $storeDataCentral)) {
							continue;
						}
						...
						?>
						<?php var_dump($value->code_address_store); ?>
						<option data-phone="<?= !empty($value->phone) ? $value->phone : "" ?>"
								data-address="<?= !empty($value->address) ? $value->address : "" ?>"
								data-code-address="<?= !empty($value->code_address_store) ? $value->code_address_store : "" ?>"
								value="<?= !empty($value->_id->{'$oid'}) ? $value->_id->{'$oid'} : "" ?>" <?= ($lead_info->id_PDG && $lead_info->id_PDG == $value->_id->{'$oid'}) ? 'selected' : ''; ?> ><?= !empty($value->name) ? $value->name : "" ?></option>
					<?php } ?>
				</select>
			</div>
		</div>
		...
		<button class="btn btn-primary  pull-right save_contract" type="button" data-toggle="modal" data-step="5"

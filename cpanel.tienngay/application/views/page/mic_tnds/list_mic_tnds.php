		...
		<div class="col-xs-12">
			...
			<div class="page-title">
				<div class="title_left">
					<h3>Danh sách thu tiền MIC_TNDS</h3>
				</div>
				<div class="title_right text-right">
					<a href="<?php echo base_url() ?>mic_tnds/form_add_mic_tnds" class="btn btn-info">
						<i class="fa fa-plus" aria-hidden="true"></i>
						Thêm giao dịch
					</a>
					<a href="<?php echo base_url() ?>mic_tnds/add_transaction_pay_money" class="btn btn-success"
					   target="_blank">
					...
					<div class="dropdown" style="display:inline-block">
						...
						<ul id="lockdulieu" class="dropdown-menu dropdown-menu-right"
							...
							<div class="row">
								<form action="<?php echo base_url('mic_tnds') ?>" method="get"
									  style="width: 100%">
									...
									<div class="col-xs-12 col-md-6">
										<div class="form-group">
											...
											<input type="text" name="code_mic_tnds" class="form-control">
										</div>
									</div>
									<?php } ?>
					...
					if ($userSession['is_superadmin'] == 1 || in_array('van-hanh', $groupRoles) || in_array('hoi-so', $groupRoles) || in_array('ke-toan', $groupRoles) || in_array('quan-ly-khu-vuc', $groupRoles)) { ?>
						<a href="<?php echo base_url() ?>excel/exportMicTnds?<?= 'tab='. $tab . '&fdate=' . $fdate . '&tdate=' . $tdate . '&customer_name=' . $customer_name . '&customer_phone=' . $customer_phone . '&code=' . $code . '&code_mic_tnds=' . $code_mic_tnds . '&filter_by_store=' . $filter_by_store;?>" class="btn btn-success"
						   target="_blank">
							<i class="fa fa-save" aria-hidden="true"></i>
							Xuất Excel
						</a>
					...
					<div class="row">
						...
						<div class="col-xs-12 text-right">
							...
							<ul id="myTab1" class="nav nav-tabs bar_tabs left" role="tablist">
								<li role="presentation" class="<?= ($tab == 'mic_tnds') ? 'active' : '' ?>">
									<a href="<?php echo base_url() ?>mic_tnds?tab=mic_tnds"
									   id="naptientaixeheyu1-tabb"
									   aria-expanded="true"> Giao dịch KH</a>
								</li>
								<li role="presentation" class="<?= ($tab == 'transaction') ? 'active' : '' ?>"><a
											href="<?php echo base_url() ?>mic_tnds?tab=transaction"
				...
				<div class="x_content">
					<div class="" role="tabpanel" data-example-id="togglable-tabs">
						<div id="myTabContent2" class="tab-content">
							<div role="tabpanel" class="tab-pane fade in <?= ($tab == 'mic_tnds') ? 'active' : '' ?>"
								 id="naptientaixeheyu1"
								 aria-labelledby="naptientaixeheyu1-tab">
								<?php if ($tab == 'mic_tnds') { ?>
									<div ><?php echo "Kết quả: " . $result_count;?></div>
											...
											<?php if (!empty($transaction)) : ?>
												<?php foreach ($transaction as $key => $value) { ?>
													<tr style="text-align: center">
														<td><?php echo !empty($value->mic_code) ? $value->mic_code : '' ?></td>
														...
														<td><?php echo !empty($value->mic_fee) ? number_format($value->mic_fee) . " VND" : '' ?></td>
														...
														<td>
															<a class="btn btn-success btn-sm" target="_blank" href="<?php echo $value->FILE ?>">Xem</a>
														</td>
													</tr>
												<?php } ?>
											<?php else : ?>
											...
											<?php if (!empty($transaction)) : ?>
												<?php foreach ($transaction as $value) { ?>
													<tr style="text-align: center">
														<td>
															<div class="dropdown">
																...
																<ul class="dropdown-menu"
																	style="z-index: 99999;">
																	<li>
																		<a href="<?php echo base_url() ?>mic_tnds/detail_transaction?code=<?php echo $value->code ?>">Xem
																			chi tiết
																		</a>
																	</li>
																	<?php
														...
														<td><?= !empty($value->created_by) ? $value->created_by : "" ?>
															<br>
															<?php
...
<script src="<?php echo base_url() ?>assets/js/mic_tnds/index.js"></script>

...
//        die;
    ...
    if(!empty($contractDataTatToan)){
        ...
        foreach($contractDataTatToan as $key => $contract){
            $tien_tra_phai_tra_hang_thang = 0;
            $tien_goc_1ky = 0;
            $tien_lai_1ky = 0;
            $tien_phi_tham_dinh_luu_tru_tai_san_1ky = 0;
            $phat_cham_tra_1ky = !empty($contract->fee_delay) ? $contract->fee_delay : 0;
            $phat_tat_toan = !empty($contract->fee_finish_contract) ? $contract->fee_finish_contract : 0;
            $tong_tien_phai_thanh_toan = 0;
            if(!empty($contract->ki_khach_hang_tat_toan) && $contract->ki_khach_hang_tat_toan == 1) {
                $tien_tra_phai_tra_hang_thang = (float)$contract->so_tien_goc_da_tra_tat_toan + 
                                (float)$contract->so_tien_lai_da_tra_tat_toan + 
                                (float)$contract->so_tien_phi_da_tra_tat_toan + 
                                (float)$contract->fee_delay + 
                                (float)$contract->tien_thua_tat_toan +
                                (float)$total_transaction_tat_toan_goc_da_tra + 
                                (float)$total_transaction_tat_toan_lai_da_tra + 
                                (float)$total_transaction_tat_toan_phi_da_tra;
                $tien_goc_1ky = (float)$contract->so_tien_goc_da_tra_tat_toan + $total_transaction_tat_toan_goc_da_tra;
                $tien_lai_1ky = (float)$contract->so_tien_lai_da_tra_tat_toan + $total_transaction_tat_toan_lai_da_tra;
                $tien_phi_tham_dinh_luu_tru_tai_san_1ky = (float)$contract->so_tien_phi_da_tra_tat_toan + (float)$contract->tien_thua_tat_toan + $total_transaction_tat_toan_phi_da_tra;
                $tong_tien_phai_thanh_toan = $tien_goc_1ky + $tien_lai_1ky + $tien_phi_tham_dinh_luu_tru_tai_san_1ky + $phat_cham_tra_1ky + $phat_tat_toan;
            } else {
...
//                        $tien_lai_1ky = 0;
//                        $tien_phi_tham_dinh_luu_tru_tai_san_1ky = 0;
//                        $tong_tien_phai_thanh_toan = $tien_tra_phai_tra_hang_thang + $phat_cham_tra_1ky;
//                    } else {
                        ...
                        <td>
                                    ...
                                    if ($contract->status == 1) {
                                            ...
                                            if ($time <= 0) {
                                                    ...
                                                    echo '<i class="fa fa-circle text-danger" aria-hidden="true"></i> Quá hạn '.$time.' ngày - xấu cấp 1';

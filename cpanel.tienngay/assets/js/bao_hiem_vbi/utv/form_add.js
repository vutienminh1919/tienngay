(function () {
	...
	var constraints = {
		...
		cmt_ngay_cap_nguoi_bh: {
			presence: {
				message: "^CMT ngày cấp không để trống! "
			}
		},
		cmt_noi_cap_nguoi_bh: {
			presence: {
				message: "^CMT nơi cấp không để trống! "
			}
		},
		diachi_nguoi_bh: {
			presence: {
...
$(document).ready(function () {
	$(".ban_bao_hiem_vbi_utv").click(function (event) {
		...
		if (confirm("Bạn có chắc chắn muốn bán Bảo hiểm VBI Ung thư vú?")) {
			...
			$.ajax({
				url: _url.base_url + 'baoHiemVbi/fees_apply',
				type: "POST",
				data: formData,
				dataType: 'json',
				processData: false,
				...
				success: function (data) {
					...
					if (data.code == 200) {
						...
						setTimeout(function () {
							window.location.replace(_url.base_url + "baoHiemVbi/utv");
						}, 500);
	...
	// 	$.ajax({
	// 		url: _url.base_url + 'baoHiemVbi/check_birthday?ngaysinh_nguoi_bh=' + ngaysinh_nguoi_bh,
	// 		type: "GET",
	// 		dataType: 'json',
	// 		success: function (data) {
	// 			console.log(data)
	...
	// 				$('#goi_bao_hiem').append($('<option>', {value: '', text: "Chon gói BH:"}));
	...
	$('#goi_bao_hiem').change(function () {
		...
		$.ajax({
			url: _url.base_url + 'baoHiemVbi/get_price_goi_bh',
			type: "POST",
			data: {
